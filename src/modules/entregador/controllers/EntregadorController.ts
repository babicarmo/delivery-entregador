...
export class EntregadorController {
  ...
  async buscarPorPedido(req: FastifyRequest<{ Params: { pedidoId: string } }>, reply: FastifyReply) {
    try {
      const { pedidoId } = req.params
      const entregador = await service.buscarPorPedido(pedidoId)
      return reply.send(entregador)
    } catch (err) {
      return tratarErro(err, reply)
    }
  ...
  async atribuirPedido(req: FastifyRequest<{ Params: { id: string } }>, reply: FastifyReply) {
    try {
      const { id } = req.params
      const { pedidoId } = req.body as { pedidoId: string }
      if (!pedidoId) {
        return reply.status(400).send({ error: 'pedidoId é obrigatório' })
      }
      const entregador = await service.atribuirPedido(id, pedidoId)
      return reply.send(entregador)
    } catch (err) {
      return tratarErro(err, reply)
    }
  ...
  async concluirEntrega(req: FastifyRequest<{ Params: { id: string } }>, reply: FastifyReply) {
    try {
      const { id } = req.params
      const entregador = await service.concluirEntrega(id)
      return reply.send(entregador)
    } catch (err) {
      return tratarErro(err, reply)
    }
